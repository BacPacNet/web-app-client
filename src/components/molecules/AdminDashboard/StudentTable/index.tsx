...
import StudentStatusBadge from '@/components/molecules/AdminDashboard/StudentStatusBadge'
import StudentYearBadge from '@/components/molecules/AdminDashboard/StudentYearBadge'
  ...
  return (
    <div className={`flex flex-col gap-3 ${className}`}>
      ...
      <div className="overflow-hidden rounded-xl border border-neutral-200 bg-white shadow-sm">
        <div className="grid grid-cols-[40px_minmax(180px,1.4fr)_minmax(160px,1.2fr)_minmax(120px,1fr)_100px_110px_48px] items-center gap-3 border-b border-neutral-200 bg-neutral-50 px-4 py-3 text-xs font-semibold uppercase tracking-wide text-neutral-500">
        ...
        {!isLoading &&
          ...
          users.map((user) => {
            ...
            const inactiveTextClass = isActive ? 'text-neutral-900' : 'text-neutral-400'

            return (
              <div
                key={user._id}
                className={`grid grid-cols-[40px_minmax(180px,1.4fr)_minmax(160px,1.2fr)_minmax(120px,1fr)_100px_110px_48px] items-center gap-3 border-b border-neutral-100 px-4 py-3 text-sm last:border-none ${
              ...
              >
                ...
                <div className="flex min-w-0 items-center gap-3">
                  ...
                  <p className={`truncate font-semibold ${inactiveTextClass}`}>
                    {user.firstName} {user.lastName}
                  </p>
                </div>

                <p className={`truncate text-neutral-500 ${!isActive ? 'text-neutral-400' : ''}`}>{getDisplayEmail(user)}</p>
                <p className={`truncate ${inactiveTextClass}`}>{user.profile?.major || '-'}</p>
                <StudentYearBadge year={user.profile?.study_year} />
                <StudentStatusBadge isActive={isActive} />

                <div className="flex justify-end">
                  <StudentRowActionMenu
                    isActive={isActive}
                    onDeactivate={onDeactivate ? () => onDeactivate(user._id) : undefined}
