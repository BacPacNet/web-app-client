...
import { useGetCommentById, useLikeUnlikeUserPostComment } from '@/services/community-timeline'
import { useGetCommunityCommentById, useLikeUnlikeGroupPostComment } from '@/services/community-university'
...
const NestedCommentModal = ({ reply, setReply, type, sortBy }: reply) => {
  ...
  const likePostCommentHandler = (commentId: string, level: string) => {
    if (type === PostType.Timeline) {
      likeUserPostComment({ userPostCommentId: commentId, level, isLiked: false })
    } else if (type === PostType.Community) {
      likeGroupPostComment({ communityGroupPostCommentId: commentId, level, isLiked: false })
    }
  }
